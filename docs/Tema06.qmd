...
ventas <- 00 + .8 * (renta-1500) -0.0005 * (renta-1500)^2 + 450 * descuento +
  #50 * descuento * (renta < 2500) +  200 * descuento * (renta < 1000) +
  - 0.25 * descuento * (renta-1250)  +  
  # 100 * descuento * (renta < 1000) +   
  100 * (zona == "ciudad") + 500 * (zona=="capital") +
  rnorm(n, mean = 0, sd = 200)
...
datos %>% ggplot(aes(x=renta, y=ventas)) + geom_point() + 
...
modelo1b <- lm(ventas ~ renta + I(renta^2) + descuento , data = datos)
summary(modelo1b)
...
```{r}
lm(data=descuento, ventas ~ renta + I(renta^2) + descuento) %>% summary()
ggplot(descuento, aes(x=renta, y=ventas)) + geom_point() + 
  geom_smooth(method = "lm", formula = y ~ x + I(x^2))
```
...
```{r}
lm(data=descuento, ventas ~ descuento  + 
     cut(renta, seq(0, 4000, 1000), include.lowest = T)) %>% summary()
ggplot(descuento, aes(x=renta, y=ventas)) + geom_point() + 
  geom_smooth(method = "lm", formula = y ~ cut(x, seq(0, 4000, 1000), include.lowest = T))
```
